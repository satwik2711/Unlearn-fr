"""Direct reversible target-suppression objective and base fingerprinting."""
...
def suppression_pair_loss(
    ...
    *,
    ...
    correct = [
        encode_answer(tokenizer, pair["forget_question"], pair["correct_answer"])
        for pair in pairs
    ]
    retain = [
    ...
    losses = _per_example_losses(model, tokenizer, [*refusal, *correct, *retain])
    size = len(pairs)
    refusal_nll = losses[:size]
    correct_nll = losses[size : 2 * size]
    retain_nll = losses[2 * size :]
    refusal_scores = -refusal_nll
    correct_scores = -correct_nll
    margin_violations = functional.relu(
        margin_delta + correct_scores - refusal_scores
    )
    refusal_loss = refusal_nll.mean()
    margin_loss = margin_violations.mean()
    retain_loss = retain_nll.mean()
    objective = (
        refusal_loss
        + margin_lambda * margin_loss
        + retain_lambda * retain_loss
    )
    return {
        "objective": objective,
        ...
        "correct_score": correct_scores.mean(),
        "refusal_score": refusal_scores.mean(),
        "satisfied_fraction": margin_violations.eq(0).float().mean(),
    }
