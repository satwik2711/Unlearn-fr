#!/usr/bin/env python3
"""Evaluate IDK checkpoints, select one on discovery, and audit reversibility."""
...
from pivot_experiment.idk import file_sha256, select_idk_checkpoint  # noqa: E402
...
from pivot_experiment.records import read_unique  # noqa: E402
...
def main() -> None:
    ...
    training_summary_path = args.artifacts / "results" / "idk_training_summary.json"
    if not training_summary_path.exists():
        raise SystemExit("IDK training summary is missing")
    training_summary = json.loads(training_summary_path.read_text(encoding="utf-8"))
    if training_summary.get("status") != "complete":
        raise SystemExit("IDK training is not complete")
    ...
    print(f"IDK candidates: {[row['adapter_id'] for row in candidates]}")
    ...
    for candidate in candidates:
        ...
        output_root = args.artifacts / "scores" / "idk_candidates"
        evaluate_subset(
            model=model,
            tokenizer=tokenizer,
            state="idk",
            ...
            activation_dir=args.artifacts / "activations" / "idk_candidates",
    ...
    selection = select_idk_checkpoint(experiment, args.artifacts, candidates)
    ...
    atomic_json(args.artifacts / "results" / "idk_reversibility.json", reversibility)
    del model
    gc.collect()
    if torch.backends.mps.is_available():
        torch.mps.empty_cache()
    print(f"Selected {selected_name}; IDK evaluation and reversibility audit complete")
