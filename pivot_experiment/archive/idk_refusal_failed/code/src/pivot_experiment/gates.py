...
def check_p1(config: dict, artifact_root: Path, p0_result: dict | None = None) -> dict:
    ...
    if not p0_result or p0_result.get("status") != "PASS":
        ...
        return result
    try:
        selection_path = artifact_root / "results" / "idk_selection.json"
        reversibility_path = artifact_root / "results" / "idk_reversibility.json"
        training_path = artifact_root / "results" / "idk_training_summary.json"
        ...
        if reversibility.get("status") != "complete" or training.get("status") != "complete":
            raise RuntimeError("IDK training or reversibility audit is incomplete")
