...
def prepare_idk_training_pairs(idk_config: dict, artifact_root: Path) -> dict:
    """Freeze one refusal target and one retain example per forget10 row."""
    ...
    output = artifact_root / "data" / "idk_training.jsonl"
    manifest_path = artifact_root / "manifests" / "idk_training_data.json"
    if output.exists() or manifest_path.exists():
        existing_rows = read_jsonl(output)
        existing_manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        if existing_rows != pairs or existing_manifest != manifest:
            raise ValueError("Existing IDK training data differs; refusing overwrite")
        return manifest
