...
def decoder_layers(model):
    """Return Llama decoder layers for a plain or PEFT-wrapped causal LM."""
    candidates = (
        lambda: model.model.layers,
        lambda: model.base_model.model.model.layers,
        lambda: model.get_base_model().model.layers,
    )
    for candidate in candidates:
        ...
        if layers is not None:
            return layers
    raise AttributeError("Could not locate Llama decoder layers")
